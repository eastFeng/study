...
//        interrupt1();
    ...
    /**
     * start:
     * 1. 开启一个新线程，在新的线程运行run方法中的代码
     * 2. start方法只是让线程进入就绪，里面的代码不一定立刻运行(CPU的时间片还没分给它)
     * 3. 每个线程的start方法只能调用一次，如果调用多次会抛出IllegalThreadStateException异常
     *
     * getState: 获取线程状态
     * Java语言一共定义了6种状态，在任意一个时间点，一个线程只能有且只有其中的一个状态
     *           NEW : 创建后尚未启动
     *      RUNNABLE : 有可能正在执行，也有可能正在等待着操作系统为它分配执行时间
     *       BLOCKED : 阻塞状态，
     *       WAITING : 无限期等待，这种状态的线程不会被分配处理器执行时间，他们要等待被其他线程显示的唤醒。
     * TIMED_WAITING : 限期等待，这种状态的线程也不会被分配处理器执行时间，不过无须等待被其他线程显示唤醒，在一定时间之后它们会由操作系统自动唤醒
     *    TERMINATED : 线程已经结束执行
     */
...
//        t1.start();
    ...
    /**
     * sleep:
     * 1. 调用sleep方法会让【当前线程】从Running进入Timed Waiting状态---->哪个线程调用，哪个线程休眠
     * 2. 其他线程可以使用interrupt方法打断正在睡眠的线程，这时sleep方法会抛出InterruptedException异常
     * 3. 睡眠结束后的线程未必会立刻得到执行(等待CPU分配时间片给它)
     * 4. 建议使用TimeUnit的sleep方法代替Thread的sleep来获得更好的可读性
     *
     * yield:
     * 1. 调用yield方法会让当前线程从Running状态进入Runnable状态，然后调度执行其他同优先级的线程。
    ...
    /**
     * setPriority: 设置线程优先级
     * 1. 线程优先级会提示调度器优先调度该线程，但它仅仅是个提示，调度器可以忽略它
     * 2. 如果CPU比较忙，那么优先级高的线程会获得更多的时间片，但CPU闲的时候，优先级几乎没什么用
     */
...
//        t2.setPriority(Thread.MAX_PRIORITY);
    ...
    public static void test(){
        ...
        Thread t1 = new Thread(() -> {
            log.info("【开始...】");
            try {
                log.info("【休眠 sleep 开始...】");
                TimeUnit.SECONDS.sleep(1);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            log.info("【休眠 sleep 技术...】");
            r = 10;
            log.info("【结束】");
        });
        t1.start();
     ...
     * 【最多】：
     * 1.如果已经等了millis毫秒线程还没有结束，就不会继续等待了
     * 2.如果还没有等够millis毫秒线程就结束了，也不会继续等了
     */
