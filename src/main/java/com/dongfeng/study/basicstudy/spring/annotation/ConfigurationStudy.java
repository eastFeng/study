...
public class ConfigurationStudy {
     ...
     *
     * <p>2. {@link Configuration}注解标记在类上（代表告诉springboot这个类是一个配置类）。
     * 相当于我们原来写的spring的配置文件。
     *
     * <p>3. {@link Configuration}标注在类上，配置Spring容器(应用上下文)。
     * 相当于把该类作为Spring的xml配置文件中的<beans>。
     * 在被{@link Configuration}注解标注的类中，使用{@link Bean}注解标注的方法，方法返回的类型都会直接注册为bean。
     ...
     * 被注解的类内部包含有一个或多个被{@link Bean}注解的方法，
     * 这些方法将会被AnnotationConfigApplicationContext或AnnotationConfigWebApplicationContext类进行扫描，
     * 并用于构建bean定义（一般使用{@link Bean}注解标注在方法上来定义bean），初始化Spring容器。
     *
     ...
     * SpringBoot不是Spring的加强版，所以{@link Configuration}与{@link Bean}同样可以用在普通的spring项目中，
     ...
     * 本项目的启动类为{@link StudyApplication}。
     *
     */
    @Bean
