...
public class Servlet_01 extends HttpServlet {
     ...
     *
     * <p>
     * 二. <b>在Servlet规范中，HTTP服务器(比如Tomcat)能调用的【动态资源文件】
     * 必须是{@link Servlet}接口的实现类。</b>
     * </p>
     *
     * <p> 三. {@link Servlet}接口实现类开发步骤 </p>
     * <p>【注意】</p>
     * <p> 1. Servlet可以在任何协议下访问，写的servlet必须实现Servlet接口。</p>
     * <p> 2. {@link Servlet}接口的实现类，
     * 最主要的是实现其中的{@link Servlet#service(ServletRequest, ServletResponse)}方法，
     * 因为该方法是收到请求后处理请求的方法（负责响应客户端的请求），
     * 当容器接收到客户端要求访问特定Servlet对象的请求时，会调用该Servlet对象的service()方法，每次请求都会执行service方法。
     * <p> 2. 而为了简化开发，官方中提供了一个实现Servlet接口的抽象类{@link GenericServlet}，
     ...
     *
     * <p>
     * 3. 官方针对HTTP协议（Web访问）专门提供了一个抽象类：{@link HttpServlet}。
     * 该类继承于{@link GenericServlet}类，在其基础上针对HTTP的特点进行扩充。
     * <p> 该类重写了{@link HttpServlet#service(ServletRequest, ServletResponse)}方法【一定要看看源码】，
     * 将{@link Servlet}定义的参数{@link ServletRequest}和{@link ServletResponse}
     * 分别转换成了{@link HttpServletRequest}和{@link HttpServletResponse}，
     * 并调用了自己的方法：{@link HttpServlet#service(HttpServletRequest, HttpServletResponse)}，
     * 在{@link HttpServlet#service(HttpServletRequest, HttpServletResponse)}方法中
     * 根据不同的请求方式进行分发，分别调用了
     ...
     * {@link HttpServlet#doHead(HttpServletRequest, HttpServletResponse)}等方法处理请求。
     * <p> 【因此】，我们一般情况下写Servlet接口实现类都是继承自HttpServlet抽象类的，在写请求处理逻辑时，
     * 只需要重写HttpServlet抽象类的doGet和doPost等方法即可，并在doGet和doPost等方法中写请求处理逻辑（响应客户端请求）。
     * 父类HttpServlet会决定何种情况下调用子类的doGet和doPost等方法。模板设计模式）
     * <p>
     * 所以开发步骤是：
     * <ol>
     *     <li>创建一个Java类继承{@link HttpServlet}抽象类，
     *         使之成为{@link Servlet}接口实现类。
     *     </li>
     *     <li>重写父类{@link HttpServlet}的doGet、doPost等方法。</li>
